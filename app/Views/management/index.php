...
<?php
    ...
    $executiveHeads = array_filter($members, fn($member) =>
        stripos((string) $member['position'], 'District Commissioner') !== false
    );

    $directors = array_filter($members, fn($member) =>
        stripos((string) $member['position'], 'Director') !== false &&
        stripos((string) $member['position'], 'District Commissioner') === false
    );

    $otherManagement = array_filter($members, fn($member) =>
        stripos((string) $member['position'], 'Director') === false &&
        stripos((string) $member['position'], 'District Commissioner') === false
    );

    // Modern reusable management profile card
    $renderMember = function (array $member, string $iconClass = 'fas fa-user-tie') {
        $photo = $member['photo'] ?: 'image/cropped-BDC-site-logo.png';
       $department = $member['department'] ?? '';
        $position = $member['position'] ?? '';
        $displayTitle = !empty($department) ? $department : $position;
        $modalTitle = trim($position . (!empty($department) ? ' - ' . $department : ''));
        ?>
        <div class="col-sm-6 col-md-4 col-lg-3">
            <div class="card h-100 border-0 shadow-sm p-3 text-center bg-white profile-card hover-up position-relative"
                ...
                <?php if (stripos((string) $member['position'], 'District Commissioner') !== false): ?>
                    <span class="position-absolute top-0 end-0 badge text-white m-2 px-3 py-2"
                          style="background-color:#0a2540; font-size:0.7rem;">
                        Executive Head
                    </span>
                <?php endif; ?>
                ...
                <div class="card-body p-2 d-flex flex-column">
                    <h5 class="fw-bold text-dark h6 mb-1"><?= esc($member['name']) ?></h5>

                    <p class="small text-primary text-uppercase fw-semibold tracking-wider mb-3"
                       style="font-size: 0.75rem;">
                        <i class="<?= esc($iconClass) ?> me-1"></i><?= esc($displayTitle) ?>
                    </p>

                    <div class="mt-auto pt-2 text-muted small border-top"
                         style="font-size: 0.75rem;">
    ...
    <div class="container">
        ...
        <div class="row justify-content-center mb-5">
            <div class="col-lg-10 text-center">
                <h1 class="fw-bold text-dark display-6 mt-2 mb-3">
                    Executive Management
                </h1>

                <p class="text-dark small mx-auto" style="max-width: 850px; line-height: 1.8;">
                    The Council Management is headed by the District Commissioner.
                    The Council has several directorates and administrative departments
                    led by qualified professionals committed to delivering quality
                    public services, transparent governance, and sustainable district development.
                </p>
            </div>
        </div>
                ...
                <div class="col-lg-10">
                    <div class="card bg-white text-dark border-0 shadow-sm p-4 text-center rounded-3">
                        <p class="text-muted mb-0">
                            No management members have been published yet.
                        </p>
                    </div>
                </div>
            </div>
        ...
        <?php else: ?>
            ...
            <!-- Executive Head -->
            <?php if (!empty($executiveHeads)): ?>
                <div class="row justify-content-center mb-5">
                    <div class="col-12">

                        <div class="section-header d-flex justify-content-between align-items-end mb-4 border-bottom pb-2">
                            <h2 class="section-title text-dark fw-bold m-0 h4">
                                Executive Head
                            </h2>
                        </div>

                        <p class="text-muted small mb-4" style="max-width: 800px;">
                            The District Commissioner serves as the chief executive officer
                            responsible for overseeing council administration, policy implementation,
                            development coordination, and service delivery across the district.
                        </p>

                        <div class="row g-4 justify-content-center">
                            <?php foreach ($executiveHeads as $member): ?>
                                <?php $renderMember($member, 'fas fa-crown'); ?>
                            <?php endforeach; ?>
                        </div>
                ...
                <div class="row justify-content-center mb-5">
                    <div class="col-12">
                        ...
                        <p class="text-muted small mb-4">
                            The following directors lead key council departments and strategic operations:
                        </p>

                        <div class="row g-4">
                            <?php foreach ($directors as $member): ?>
                                <?php $renderMember($member, 'fas fa-building'); ?>
                            <?php endforeach; ?>
                        </div>
            ...
            <?php if (!empty($otherManagement)): ?>
                <div class="row justify-content-center">
                    <div class="col-12">
                        ...
                        <div class="row g-4">
                            <?php foreach ($otherManagement as $member): ?>
                                <?php $renderMember($member, 'fas fa-user-tie'); ?>
                            <?php endforeach; ?>
                        </div>
