...
<head>
    ...
    <title>Edit Business Type - Blantyre District Council</title>
        ...
        <div class="row">
            ...
            <main class="col-md-10 ms-sm-auto px-4 py-4">
                <div class="d-flex justify-content-between align-items-center pb-3 mb-4 border-bottom">
                    <div>
                        <h1 class="h3 mb-0 fw-bold">Edit Business Type</h1>
                        <p class="text-muted small mb-0">Update the business type shown on the public business license application form.</p>
                    </div>
                ...
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0 fw-semibold">Business Type Details</h5>
                    </div>
                    <div class="card-body p-4">
                        <form method="POST" action="<?= base_url('admin/business-types/' . $businessType['id'] . '/edit') ?>">
                            <?= csrf_field() ?>
                            <div class="row g-3 mb-3">
                                <div class="col-md-8">
                                    <label for="name" class="form-label">Business Type Name <span class="text-danger">*</span></label>
                                    <input type="text" class="form-control" id="name" name="name" value="<?= old('name', $businessType['name']) ?>" required>
                                </div>
                                <div class="col-md-4">
                                    <label for="category" class="form-label">Category</label>
                                    <input type="text" class="form-control" id="category" name="category" value="<?= old('category', $businessType['category']) ?>" placeholder="e.g. Retail">
                                </div>
                            </div>
                            ...
                            <div class="row g-3 mb-4">
                                <div class="col-md-4">
                                    ...
                                    <input type="number" class="form-control" id="sort_order" name="sort_order" value="<?= old('sort_order', $businessType['sort_order']) ?>" min="0">
                                </div>
                                <div class="col-md-4">
                                    <label for="is_active" class="form-label">Status</label>
                                    <select class="form-select" id="is_active" name="is_active">
                                        <option value="1" <?= (string) old('is_active', $businessType['is_active']) === '1' ? 'selected' : '' ?>>Active</option>
                                        <option value="0" <?= (string) old('is_active', $businessType['is_active']) === '0' ? 'selected' : '' ?>>Inactive</option>
                                    </select>
                                </div>
                            </div>

                            <button type="submit" class="btn btn-primary px-4"><i class="fas fa-save me-2"></i>Update Business Type</button>
                        </form>
                    </div>
                </div>
            </main>
