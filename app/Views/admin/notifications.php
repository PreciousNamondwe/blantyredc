<style>
    ...
    /* Custom Government Tab Navigation Rules */
    .gov-tab-wrapper {
    ...
    .nav-tabs.gov-tabs .nav-link:hover {
        background-color: rgba(26, 51, 82, 0.05);
        border-color: var(--excel-border) var(--excel-border) transparent;
        color: var(--gov-navy-primary);
    }

    .nav-tabs.gov-tabs .nav-link.active {
        color: var(--gov-navy-primary) !important;
        background-color: #ffffff !important;
        border-color: var(--excel-border) var(--excel-border) #ffffff !important;
        border-top: 3px solid var(--gov-navy-primary) !important;
...
<div class="gov-banner-header d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
    ...
    <div class="d-flex align-items-center gap-2" id="headerActionBlock">
...
<div class="gov-tab-wrapper">
    <ul class="nav nav-tabs gov-tabs" id="govRegistryTab" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link active" id="dispatched-tab" data-bs-toggle="tab" data-bs-target="#dispatched-panel" type="button" role="tab" aria-controls="dispatched-panel" aria-selected="true">
                <i class="fas fa-paper-plane me-2 text-muted"></i>Public Notices (Dispatched)
            </button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link" id="received-tab" data-bs-toggle="tab" data-bs-target="#received-panel" type="button" role="tab" aria-controls="received-panel" aria-selected="false">
                <i class="fas fa-inbox me-2 text-muted"></i>Notifications Received (Read-Only)
            </button>
        </li>
    </ul>
</div>
...
<div class="excel-card-container p-3 p-md-4 mb-5">
    ...
    <div class="tab-content" id="govRegistryTabContent">
        
        <div class="tab-pane fade show active" id="dispatched-panel" role="tabpanel" aria-labelledby="dispatched-tab">
        ...
        <div class="tab-pane fade" id="received-panel" role="tabpanel" aria-labelledby="received-tab">
            <div class="panel-card-wrapper">
                <div class="table-responsive">
                    <table class="table excel-grid-table align-middle">
                        <thead>
                            <tr>
                                <th class="ps-3" style="width: 140px;">Transmission ID</th>
                                <th>Inbound Content Summary</th>
                                <th>Origin Unit</th>
                                <th>Received Timestamp</th>
                                <th class="pe-3 text-end" style="width: 120px;">Classification</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php if (!empty($received_notifications) && is_array($received_notifications)) : ?>
                                <?php foreach ($received_notifications as $msg) : ?>
                                    <tr>
                                        <td class="ps-3 text-muted font-monospace small fw-bold">
                                            <?= esc($msg['reference'] ?? 'TRX-INB-'.rand(1000,9999)) ?>
                                        </td>
                                        <td>
                                            <div class="fw-bold text-dark mb-0" style="font-size: 0.85rem;"><?= esc($msg['title']) ?></div>
                                            <span class="text-muted small" style="font-size: 0.78rem;"><?= esc($msg['content']) ?></span>
                                        </td>
                                        <td>
                                            <span class="badge bg-light text-secondary border px-2 py-1 rounded small fw-semibold">
                                                <i class="fas fa-university me-1"></i><?= esc($msg['sender_unit'] ?? 'Central Authority') ?>
                                            </span>
                                        </td>
                                        <td class="text-muted font-monospace small">
                                            <?= esc($msg['created_at'] ?? date('Y-m-d H:i')) ?>
                                        </td>
                                        <td class="pe-3 text-end">
                                            <span class="status-pill px-2 py-1 bg-info text-dark bg-opacity-10">
                                                <i class="fas fa-envelope-open-text me-1"></i>Inbound
                                            </span>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else : ?>
                                <tr>
                                    <td colspan="5" class="text-center py-5 text-muted">
                                        <div class="py-3">
                                            <i class="fas fa-inbox d-block fs-3 mb-2 opacity-35" style="color: var(--gov-navy-primary);"></i>
                                            <span class="small fw-medium">No inbound external notifications recorded in this session profile.</span>
                                        </div>
                                    </td>
                                </tr>
                            <?php endif; ?>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const headerActionBlock = document.getElementById('headerActionBlock');

        if (dispatchedTab && receivedTab && headerActionBlock) {
            // Hide Dispatch Action button when viewing inbound logs
            receivedTab.addEventListener('shown.bs.tab', function () {
                headerActionBlock.style.setProperty('display', 'none', 'important');
            });

            // Restore primary dispatch operation button when viewing master directory
            dispatchedTab.addEventListener('shown.bs.tab', function () {
                headerActionBlock.style.setProperty('display', 'flex', 'important');
            });
        }
    });
</script>